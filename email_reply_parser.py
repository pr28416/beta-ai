...
class Email(object):
    ...
    def scan_line(self, line):
        ...
        if self.fragment and (
            (self.fragment.quoted == is_quoted)
            or self.fragment.quoted
            and (self.quote_header(line) or line == self.empty)
        ):
    ...
    def finish_fragment(self):
        ...
        if frag:
            ...
            if not self.found_visible:
                if frag.quoted or frag.signature or str(frag).strip() == self.empty:
                    frag.hidden = True
                else:
